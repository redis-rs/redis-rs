...
`SendError` now carries a message describing why the delivery failed (e.g. "the receiving end of the channel was dropped"). `SendError::new()` is unchanged.

For the `Fn(PushInfo) -> Result<(), T>` push sender, the closure's error type `T` must now implement `std::fmt::Display`, so that its message can be surfaced through `SendError`. Undeliverable push messages are now logged at `warn` level via the `log` crate, if the crate is built with the `log` dependency.
...
* If you implemented `AsyncPushSender`, you'll need to define a printable error types.
* Push delivery failures are no longer silent: they are reported through the `SendError` value and logged as warnings. Expect these warnings when a push receiver is dropped while the connection is still active.
