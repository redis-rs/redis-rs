...
fn test_json_arr_index() {
    ...
    let json_arr_index: RedisResult<Value> = con.json_arr_index(TEST_KEY, "$..a", &2i64);

    assert_eq!(json_arr_index, Ok(Bulk(vec![Int(1i64), Int(-1i64)])));
...
fn test_json_arr_insert() {
    ...
    let json_arr_insert: RedisResult<Value> = con.json_arr_insert(TEST_KEY, "$..a", 0, &1i64);

    assert_eq!(json_arr_insert, Ok(Bulk(vec![Int(2), Int(3)])));
    ...
    let json_arr_insert_2: RedisResult<Value> = con.json_arr_insert(TEST_KEY, "$..a", 0, &1i64);

    assert_eq!(json_arr_insert_2, Ok(Bulk(vec![Int(5), Nil])));
}
...
fn test_json_arr_len() {
    ...
    let json_arr_len: RedisResult<Value> = con.json_arr_len(TEST_KEY, "$..a");

    assert_eq!(json_arr_len, Ok(Bulk(vec![Int(1), Int(2)])));
    ...
    let json_arr_len_2: RedisResult<Value> = con.json_arr_len(TEST_KEY, "$..a");

    assert_eq!(json_arr_len_2, Ok(Bulk(vec![Int(4), Nil])));
}
...
fn test_json_arr_pop() {
    ...
    let json_arr_pop: RedisResult<Value> = con.json_arr_pop(TEST_KEY, "$..a", -1);
    ...
    let json_arr_pop_2: RedisResult<Value> = con.json_arr_pop(TEST_KEY, "$..a", -1);
...
fn test_json_arr_trim() {
    ...
    let json_arr_trim: RedisResult<Value> = con.json_arr_trim(TEST_KEY, "$..a", 1, 1);

    assert_eq!(json_arr_trim, Ok(Bulk(vec![Int(0), Int(1)])));
    ...
    let json_arr_trim_2: RedisResult<Value> = con.json_arr_trim(TEST_KEY, "$..a", 1, 1);

    assert_eq!(json_arr_trim_2, Ok(Bulk(vec![Int(1), Nil])));
}
...
fn test_json_num_incr_by() {
    ...
    let json_num_incr_by_a: RedisResult<String> = con.json_num_incr_by(TEST_KEY, "$.a", 2);

    // cannot increment a string
    assert_eq!(json_num_incr_by_a, Ok("[null]".into()));

    let json_num_incr_by_b: RedisResult<String> = con.json_num_incr_by(TEST_KEY, "$..a", 2);

    // however numbers can be incremented
    assert_eq!(json_num_incr_by_b, Ok("[null,4,7,null]".into()));
}
...
fn test_json_obj_keys() {
    ...
    let json_obj_keys: RedisResult<Value> = con.json_obj_keys(TEST_KEY, "$..a");
...
fn test_json_obj_len() {
    ...
    let json_obj_len: RedisResult<Value> = con.json_obj_len(TEST_KEY, "$..a");

    assert_eq!(json_obj_len, Ok(Bulk(vec![Nil, Int(2)])));
}
...
fn test_json_str_append() {
    ...
    let json_str_append: RedisResult<Value> = con.json_str_append(TEST_KEY, "$..a", "\"baz\"");

    assert_eq!(json_str_append, Ok(Bulk(vec![Int(6), Int(8), Nil])));
...
fn test_json_str_len() {
    ...
    let json_str_len: RedisResult<Value> = con.json_str_len(TEST_KEY, "$..a");

    assert_eq!(json_str_len, Ok(Bulk(vec![Int(3), Int(5), Nil])));
}
