...
mod cluster_async {
    ...
    use redis::{
        aio::{ConnectionLike, MultiplexedConnection, DisconnectNotifier},
    ...
    fn validate_subscriptions(pubsub_subs: &PubSubSubscriptionInfo, notifications_rx: &mut mpsc::UnboundedReceiver<PushInfo>, allow_disconnects: bool) {
        let mut subscribe_cnt = if let Some(exact_subs) = pubsub_subs.get(&PubSubSubscriptionKind::Exact) {
            exact_subs.len()
        }
        else {
            0
        ...
        let mut psubscribe_cnt = if let Some(pattern_subs) = pubsub_subs.get(&PubSubSubscriptionKind::Pattern) {
            pattern_subs.len()
        }
        else {
            0
        ...
        let mut ssubscribe_cnt = if let Some(sharded_subs) = pubsub_subs.get(&PubSubSubscriptionKind::Sharded) {
            sharded_subs.len()
        }
        else {
            0
        ...
        for _ in 0..(subscribe_cnt + psubscribe_cnt + ssubscribe_cnt) {
            ...
            assert!(
                kind == PushKind::Subscribe
                    ...
                    || if allow_disconnects { kind == PushKind::Disconnection } else { false }
            );
    ...
    fn test_async_cluster_test_fast_reconnect() {
        ...
        block_on_all(async move {
            ...
            #[cfg(feature = "rust-comp")]
            let tries = 0..3;
            #[cfg(not(feature = "rust-comp"))]
            let tries = 0..1;
    ...
    fn test_async_cluster_restore_resp3_pubsub_state_after_scale_out() {
        ...
        block_on_all(async move {
            ...
            let last_server_port = {
                ...
                match addr {
                    redis::ConnectionAddr::TcpTls{host: _, port, insecure: _, tls_params: _} => {
                        port
                    },
            ...
            loop {
                ...
                let res = publishing_con
                    .route_command(
                        &cmd,
                        RoutingInfo::SingleNode(SingleNodeRoutingInfo::SpecificNode(
                            Route::new(slot_14212, SlotAddr::Master),
                        )),
                    )
                    .await;
                assert!(res.is_ok());
