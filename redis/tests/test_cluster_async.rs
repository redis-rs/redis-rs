...
mod cluster_async {
    ...
    use redis::{
        ...
        cluster_topology::{DEFAULT_NUMBER_OF_REFRESH_SLOTS_RETRIES, get_slot},
        ...
        RedisResult, Script, Value, PushKind, PushInfo,
        PubSubSubscriptionInfo, PubSubSubscriptionKind, PubSubChannelOrPattern,
    };
    ...
    fn test_async_cluster_resp3_pubsub() {
        ...
        let client_subscriptions = PubSubSubscriptionInfo::from(
            [
                ...
                (PubSubSubscriptionKind::Pattern, HashSet::from(
                    [
                        PubSubChannelOrPattern::from(b"test_*"),
                        PubSubChannelOrPattern::from(b"*")
                    ])
                ),
                (PubSubSubscriptionKind::Sharded, HashSet::from(
                    [
                        PubSubChannelOrPattern::from(b"test_channel_?")
                    ])
                ),
        ...
        block_on_all(async move {
            ...
            + client_subscriptions[&PubSubSubscriptionKind::Sharded].len()) {
                ...
                assert!(kind == PushKind::Subscribe || kind == PushKind::PSubscribe || kind == PushKind::SSubscribe);
            ...
            let result = connection
            .route_command(&redis::Cmd::new().arg("PUBLISH").arg("test_channel_?").arg("test_message"), RoutingInfo::SingleNode(node_0_route.clone()))
            .await;
            assert!(result.is_ok());
