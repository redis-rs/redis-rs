...
use redis_test::{assert_redis_result_err_code_eq, assert_redis_result_err_detail_contains};
use std::vec;
...
fn test_module_bloom_single_value_updates() {
    ...
    assert_redis_result_err_code_eq(con.bf_add(KEY_2, "foo"), "WRONGTYPE");
...
fn test_module_bloom_multiple_value_updates() {
    ...
    assert_redis_result_err_detail_contains(
        con.bf_insert_options(KEY_2, &["foo"], options),
        "not found",
    );
    ...
    assert_redis_result_err_detail_contains(con.bf_madd(KEY_3, &["baz"]), "full");
    assert_redis_result_err_detail_contains(con.bf_insert(KEY_3, &["baz"]), "full");
    assert_eq!(
...
fn test_module_bloom_infos() {
    ...
    assert_redis_result_err_detail_contains(con.bf_info(KEY_1), "not found");
    assert_redis_result_err_detail_contains(
        con.bf_info_type(KEY_1, BloomFilterInfoType::Items),
        "not found",
    );
    ...
    let bf_type = con
        .key_type(KEY_1)
        .expect("getting the type of a bloom filter should work");
    assert!([ValueType::BloomFilterRedis, ValueType::BloomFilterValKey].contains(&bf_type));
    assert_eq!(
        con.bf_info(KEY_1)
            .expect("bf_info should work")
    ...
    assert_eq!(con.set(KEY_2, "quux"), Ok(()));
    assert_redis_result_err_code_eq(con.bf_card(KEY_2), "WRONGTYPE");
    assert_redis_result_err_code_eq(con.bf_info(KEY_2), "WRONGTYPE");
    assert_redis_result_err_code_eq(
...
fn test_module_bloom_reserving() {
    ...
    assert_redis_result_err_detail_contains(con.bf_reserve(KEY_2, 0.4711, 42), "exists");

    // Reserving for a key with different value should fail
    assert_eq!(con.set(KEY_3, "quux"), Ok(()));
    assert_redis_result_err_code_eq(con.bf_reserve(KEY_3, 0.4711, 42), "WRONGTYPE");
...
fn test_module_bloom_dump_and_load() {
    ...
    let original_info = con
        .bf_info(KEY_1)
        .expect("getting info on the original filter should work");
    ...
    let deleted = con
        .del(KEY_1)
        .expect("deleting the original filter should succeed");
    assert_eq!(deleted, 1);

    // Load the dumped data to new filter at a different key
    for chunk in chunks {
        con.bf_loadchunk(KEY_2, chunk)
            .expect("loading the dumped data should succeed");
    }

    // Assure the original filter's items are present in the loaded filter
    let results: Vec<bool> = con
        .bf_mexists(KEY_2, &["foo", "bar"])
        .expect("checking existence should succeed");
    assert_eq!(results, vec![true, true]);

    // Assure the original and loaded filter have the same metadata
    let loaded_info = con
        .bf_info(KEY_2)
        .expect("getting info on the new filter should succeed");
