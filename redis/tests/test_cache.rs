...
fn test_cache() {
    ...
    block_on_all(async move {
        let mut con = ctx.multiplexed_async_connection_tokio_with_cache().await?;
...
#[test]
fn test_cache_drops_and_guards() {
    use redis::ProtocolVersion;
...
#[test]
fn test_cache_guards_and_different_types() {
    use redis::ProtocolVersion;
...
#[test]
fn test_cache_guards_and_nil_values() {
...
#[test]
fn test_cache_verify() {
    // Verifies task cancellation of request with same key shouldn't affect other requests to same key.
    use redis::ProtocolVersion;
