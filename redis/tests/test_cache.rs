...
fn test_cache_with_pipeline(#[case] runtime: RuntimeType, #[values(true, false)] atomic: bool) {
    ...
    block_on_all(
        async move {
            ...
            let ((k1, k2),): ((i32, i32),) = get_pipe(atomic)
                .cmd("SET")
                .arg("key_1")
                .arg(41)
                .ignore()
            ...
            let (r1, (r2, r3), r4): (i32, (i32, i32), Option<i32>) = get_pipe(atomic)
                .cmd("GET")
                .arg("key_1")
                .cmd("MGET")
                .arg(&["key_1", "key_2"])
...
fn test_cache_pipeline_partial_opt_in(
    ...
    block_on_all(
        async move {
            ...
            for _ in 0..2 {
                let ((k1, k2), k3, k4): ((i32, i32), i32, Option<i32>) = get_pipe(atomic)
                    .cmd("MGET")
                    .set_cache_config(CommandCacheConfig::new().set_enable_cache(true))
                    .arg(&["key_1", "key_2"])
                    .cmd("GET")
