...
fn test_module_search_ft_create_create_options() {
    ...
    let portable_modifiers: Vec<(&'static str, CreateOptionsModifier)> = vec![
    ...
    // Test each portable option individually
    for (suffix, modifier) in &portable_modifiers {
    ...
    // Combine the portable options cumulatively
    ...
    // valkey-search rejects the rest, so stop here for it.
    if is_valkey {
        return;
    }

    // Redis-only options.
    let redis_only_modifiers: Vec<(&'static str, CreateOptionsModifier)> = vec![
    ...
    // Test each Redis-only option individually
    for (suffix, modifier) in redis_only_modifiers
        .iter()
        .chain(&mutually_exclusive_modifiers)
    {
        let index_name = format!("index_with_{suffix}");
        let options = modifier(CreateOptions::new());
    ...
    // Continue the cumulative combination with the Redis-only options
    for (suffix, modifier) in &redis_only_modifiers {
        let combined_index_name = format!("combined_index_until_{suffix}");
        combined_options = modifier(combined_options);
...
fn run_ft_create_schema_text_field<C, F>(con: &mut C, is_valkey: bool, mut on_created: F)
where
    C: redis::ConnectionLike,
    F: FnMut(&str),
{
    ...
    let portable_modifiers: Vec<(&'static str, SchemaTextFieldModifier)> = vec![
    ...
    // Test each portable modifier individually
    for (suffix, modifier) in &portable_modifiers {
    ...
    // Combine the portable modifiers cumulatively
    ...
    let redis_only_modifiers: Vec<(&'static str, SchemaTextFieldModifier)> = vec![
    ...
    let mutually_exclusive_modifiers: Vec<(&'static str, SchemaTextFieldModifier)> = vec![
        ("index_missing", |field| field.index_missing(true)),
        ("no_index", |field| field.no_index(true)),
    ];

    // Test each Redis-only modifier individually
    for (suffix, modifier) in redis_only_modifiers
        .iter()
        .chain(&mutually_exclusive_modifiers)
    {
    ...
    // Continue the cumulative combination with the Redis-only modifiers
    for (suffix, modifier) in &redis_only_modifiers {
        let combined_index_name = format!("index_for_text_field_combined_until_{suffix}");
        combined_schema_text_field = modifier(combined_schema_text_field);
        let schema = schema! {
...
fn run_ft_create_schema_tag_field<C, F>(con: &mut C, is_valkey: bool, mut on_created: F)
where
    C: redis::ConnectionLike,
    F: FnMut(&str),
{
    ...
    // Portable modifiers — accepted by both Redis and valkey-search.
    let portable_modifiers: Vec<(&'static str, SchemaTagFieldModifier)> = vec![
    ...
    // Redis-only modifiers.
    let redis_only_modifiers: Vec<(&'static str, SchemaTagFieldModifier)> = vec![
        ("sortable_unf", |field| field.sortable(Sortable::Unf)),
        ("with_suffix_trie", |field| field.with_suffix_trie(true)),
        ("index_empty", |field| field.index_empty(true)),
    ];
    ...
    // Redis-only modifiers that are mutually exclusive
    let mutually_exclusive_modifiers: Vec<(&'static str, SchemaTagFieldModifier)> = vec![
        ("index_missing", |field| field.index_missing(true)),
        ("no_index", |field| field.no_index(true)),
    ];
...
fn run_ft_create_schema_numeric_field<C, F>(con: &mut C, is_valkey: bool, mut on_created: F)
where
    C: redis::ConnectionLike,
    F: FnMut(&str),
{
    ...
    // Portable modifiers — accepted by both Redis and valkey-search.
    let portable_modifiers: Vec<(&'static str, SchemaNumericFieldModifier)> = vec![
    ...
    // Redis-only modifiers.
    let redis_only_modifiers: Vec<(&'static str, SchemaNumericFieldModifier)> =
        vec![("sortable_unf", |field| field.sortable(Sortable::Unf))];

    // Redis-only modifiers that are mutually exclusive
    let mutually_exclusive_modifiers: Vec<(&'static str, SchemaNumericFieldModifier)> = vec![
        ("index_missing", |field| field.index_missing(true)),
        ("no_index", |field| field.no_index(true)),
    ];
