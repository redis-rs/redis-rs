...
* Validate RESP3 requirement for async push sender in async cluster connections. Client creation now returns `InvalidClientConfig` when a push sender is provided with RESP2 or unspecified protocol. Documentation updated to clarify requirement in `ClusterClientBuilder::push_sender` and async cluster module docs.
