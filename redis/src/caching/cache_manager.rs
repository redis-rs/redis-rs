...
struct ShardedLRU {
    shards: Vec<parking_lot::Mutex<LRUCacheShard>>,
}

impl ShardedLRU {
    const MAX_SHARD_SIZE: usize = 32;
    const MIN_SHARD_SIZE: usize = 1;
    fn new(total_key_size: usize) -> Self {
        let shard_size = min(
            max(total_key_size, Self::MIN_SHARD_SIZE),
            Self::MAX_SHARD_SIZE,
        );
        ...
        for _ in 0..shard_size {
            shards.push(parking_lot::Mutex::new(LruCache::new(
                NonZeroUsize::new(max(per_lru_size, 1)).expect("LRU size must be bigger than 0"),
            )));
        }
        ShardedLRU { shards }
    }
    fn get_shard(&self, key: &[u8]) -> &parking_lot::Mutex<LRUCacheShard> {
...
impl CacheManager {
    ...
    pub(crate) async fn get_with_guard_new<'a>(
        ...
        let mut receiver = {
            let mut lru_cache_better = self.lru.get_shard(cache_information.redis_key).lock();
    ...
    fn insert_with_guard_inner(
        ...
        let mut lru_cache = self.lru.get_shard(redis_key).lock();
    ...
    pub(crate) fn invalidate(&self, cache_key: &Vec<u8>) {
        if let Some(cache_holder) = self.lru.get_shard(cache_key).lock().pop(cache_key) {
            self.statistics
