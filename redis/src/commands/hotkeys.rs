...
mod tests {
    ...
    fn test_hotkeys_response_parsing_resp2() {
        ...
        let response = Value::Array(vec![
            ...
            Value::BulkString(b"all-commands-all-slots-us".to_vec().into()),
            Value::Int(5000),
            Value::BulkString(b"net-bytes-all-commands-all-slots".to_vec().into()),
            Value::Int(2048),
            Value::BulkString(b"collection-start-time-unix-ms".to_vec().into()),
            Value::Int(1700000000000),
            Value::BulkString(b"collection-duration-ms".to_vec().into()),
            Value::Int(10000),
            Value::BulkString(b"total-cpu-time-user-ms".to_vec().into()),
            Value::Int(100),
            Value::BulkString(b"total-cpu-time-sys-ms".to_vec().into()),
            Value::Int(50),
            Value::BulkString(b"by-cpu-time-us".to_vec().into()),
            Value::Array(vec![
    ...
    fn test_hotkeys_response_parsing_resp3() {
        ...
        let response = Value::Map(vec![
            ...
            (
                Value::BulkString(b"all-commands-selected-slots-us".to_vec().into()),
                Value::Int(4000),
            ),
            (
                Value::BulkString(b"net-bytes-all-commands-all-slots".to_vec().into()),
            ...
            (
                Value::BulkString(b"net-bytes-all-commands-selected-slots".to_vec().into()),
                Value::Int(1024),
            ),
            (
                Value::BulkString(b"collection-start-time-unix-ms".to_vec().into()),
    ...
    fn test_hotkeys_response_parsing_with_net() {
        ...
        let response = Value::Array(vec![
            ...
            Value::BulkString(b"collection-duration-ms".to_vec().into()),
            Value::Int(5000),
            Value::BulkString(b"total-net-bytes".to_vec().into()),
            Value::Int(8192),
            Value::BulkString(b"by-net-bytes".to_vec().into()),
            Value::Array(vec![
                Value::BulkString(b"bigkey".to_vec().into()),
                Value::Int(4096),
                Value::BulkString(b"smallkey".to_vec().into()),
                Value::Int(256),
            ]),
        ]);
