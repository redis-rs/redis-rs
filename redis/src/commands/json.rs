...
macro_rules! json_serialize_fail {
	($err: expr) => {
		Err(
			RedisError::from((
				ErrorKind::Serialize,
...
implement_json_commands! {
	...
	fn json_numincrby<K: ToRedisArgs, P: ToRedisArgs>(key: K, path: P, value: i64) {
		Ok::<Cmd, RedisError>(
			cmd("JSON.NUMINCRBY")
				.arg_take(key)
				.arg_take(path)
	...
	fn json_strappend<K: ToRedisArgs, P: ToRedisArgs, V: ToRedisArgs>(key: K, path: P, value: V) {
		Ok::<Cmd, RedisError>(
			cmd("JSON.STRAPPEND")
				.arg_take(key)
				.arg_take(path)
