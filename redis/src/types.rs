...
#[cfg(test)]
#[path = "../tests/support/shared/mod.rs"]
mod shared;
...
mod tests {
    ...
    use shared::encode::encode_value;
    ...
    fn test_redis_error_source_presence_for_io_wrapped_errors() {
        ...
        assert_eq!(io_error.kind(), crate::ErrorKind::Io);
    ...
    impl RedisParseMode {
        ...
        fn parse_redis_value<T: crate::FromRedisValue>(
            &self,
            value: crate::Value,
        ) -> Result<T, crate::ParsingError> {
            match self {
                Self::Owned => crate::FromRedisValue::from_redis_value(value),
                Self::Ref => crate::FromRedisValue::from_redis_value_ref(&value),
            }
        }
    }
    ...
    mod numeric_and_string_replies_agree {
        use crate::{FromRedisValue, Value};
    ...
    fn test_attributes() {
        ...
        let val = parse_redis_value(bytes).unwrap();
        {
            ...
            let x: Vec<i32> = crate::FromRedisValue::from_redis_value_ref(&val).unwrap();
    ...
    fn test_complex_nested_tuples() {
        for parse_mode in [RedisParseMode::Owned, RedisParseMode::Ref] {
            ...
            let res: Vec<(std::collections::HashMap<String, String>, Vec<String>)> =
                parse_mode.parse_redis_value(value).unwrap();

            let mut expected_map1 = std::collections::HashMap::new();
            expected_map1.insert("Hi1".to_string(), "Bye1".to_string());
            expected_map1.insert("Hi2".to_string(), "Bye2".to_string());

            let mut expected_map2 = std::collections::HashMap::new();
            ...
            let mut expected_map3 = std::collections::HashMap::new();
    ...
    fn test_complex_nested_tuples_resp3() {
        for parse_mode in [RedisParseMode::Owned, RedisParseMode::Ref] {
            ...
            let res: Vec<(std::collections::HashMap<String, String>, HashSet<String>)> =
                parse_mode.parse_redis_value(value).unwrap();
