...
pub use crate::cluster_slotmap::{Route, SlotAddr};
...
mod tests {
    ...
    fn test_multi_shard() {
        ...
        let routing = RoutingInfo::for_routable(&cmd);
        let mut expected = std::collections::HashMap::new();
        expected.insert(Route::new(4813, SlotAddr::ReplicaOptional), vec![2]);
        expected.insert(Route::new(5061, SlotAddr::ReplicaOptional), vec![1, 3]);
        expected.insert(Route::new(12182, SlotAddr::ReplicaOptional), vec![0]);
