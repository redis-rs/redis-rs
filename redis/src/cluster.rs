            ...
            let mut nodes: Vec<_> = item
                ...
                .filter_map(|node| {
                    if let Value::Array(node) = node {
                        ...
                        let str: Arc<str> = std::sync::Arc::from(
                            get_connection_addr(ip.into_owned(), port, tls, None).to_string(),
                        );
                        let address = node_map.entry(str.clone()).or_insert(str).clone();
