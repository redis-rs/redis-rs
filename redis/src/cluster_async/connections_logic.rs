    ...
    C: ConnectionLike + Send + Clone + Sync + Connect + 'static,
{
    if let Some(node) = node {
        ...
        match check_node_connections(&node, params, conn_type, addr).await {
            None => Ok(node),
            Some(conn_type) => connect_and_check(addr, params.clone(), None, conn_type, Some(node), push_sender)
                .await
                .get_node(),
        }
    } else {
...
{
    match future::join(
        create_connection(addr, params.clone(), socket_addr, push_sender.clone(), false),
    ...
    C: ConnectionLike + Connect + Send + Sync + 'static + Clone,
{
    ...
    .await {
        ...
        (Ok(mut user_conn), Ok(mut mngm_conn)) => {
            ...
            return ConnectAndCheckResult::Success(ClusterNode {
    ...
    C: ConnectionLike + Connect + Send + Sync + 'static + Clone,
{
    match conn_type {
        RefreshConnectionType::OnlyUserConnection => {
            let (user_conn, ip) =
                match create_and_setup_user_connection(addr, params.clone(), socket_addr, push_sender).await {
                    Ok(tuple) => tuple,
        ...
        RefreshConnectionType::OnlyManagementConnection => {
            ...
            match node {
                Some(node) => {
                    connect_and_check_only_management_conn(addr, params, socket_addr, node, push_sender).await
                }
                None => connect_and_check_all_connections(addr, params, socket_addr, push_sender).await,
            }
        }
    ...
    C: ConnectionLike + Connect + Send + 'static,
{
    ...
    C::connect(info, response_timeout, connection_timeout, socket_addr, if !is_management {push_sender} else {None}).await
}
