...
fn authenticate_cmd(connection_info: &RedisConnectionInfo, check_username: bool) -> Cmd {
    ...
    command.arg(connection_info.password.as_ref().unwrap());
    command
}
...
pub(crate) fn connection_setup_pipeline(
    ...
    if authenticate_with_resp3_cmd_index.is_some() {
        ...
        pipeline.add_command(authenticate_cmd(connection_info, check_username));
    }
